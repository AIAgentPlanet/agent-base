...
# get config address from user_service.yml
configHost=$(bash scripts/parseYaml.sh configs/user_service.yml '.app.host')
if [ "${configHost}" = "127.0.0.1" ]; then
  configHost="localhost"
fi
configPort=$(bash scripts/parseYaml.sh configs/user_service.yml '.http.port')
configAddr="${configHost}:${configPort}"
enableMode=$(bash scripts/parseYaml.sh configs/user_service.yml '.http.tls.enableMode')
schemes="http"
if [ "$enableMode" != "" ]; then
  schemes="https"
fi
...
swaggerAddr=$(grep -E '^[[:space:]]*//[[:space:]]*@host' "cmd/user_service/main.go" | awk '{print $3}')
if [[ -z "${swaggerAddr}" ]]; then
  swaggerAddr="localhost:8080"
fi
if [ "${configAddr}" != "${swaggerAddr}" ];then
  sed -i "s/${swaggerAddr}/${configAddr}/g" cmd/user_service/main.go
fi
